...
``LLM_PROVIDER`` from :class:`~fastapi_bootstrap.core.config.LLMSettings` and
...
Architecture
------------
This module lives in the *infra* layer (``fastapi_bootstrap.infra.llm``) rather
than inside a bounded context (``domains/<bc>/``) so that:

* The chat domain depends only on its ports/interfaces
  (:class:`~fastapi_bootstrap.domains.chat.ports.LLMClientProtocol`) — never on
...
:class:`~fastapi_bootstrap.core.config.LLMSettings` (``as_litellm_kwargs``).
...
Usage::
    ...
    from fastapi_bootstrap.core.config import LLMSettings, LLMProvider
...
def make_chat_litellm(
    ...
    **override_kwargs: Any,
    ...
    :class:`~fastapi_bootstrap.core.config.LLMSettings` and constructs the
    ...
    ----------
    settings:
        Pre-populated :class:`~fastapi_bootstrap.core.config.LLMSettings`
        instance.  When *None*, the global settings singleton is used (reads
        environment variables / ``.env`` file via
        :func:`~fastapi_bootstrap.core.config.get_settings`).
    **override_kwargs:
