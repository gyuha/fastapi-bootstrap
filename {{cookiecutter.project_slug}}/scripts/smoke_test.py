...
검증 흐름:
  1. 헬스체크         : GET  /health
  2. 회원가입         : POST /api/v1/auth/register
  3. 이메일 인증      : Mailpit API → 토큰 파싱 → GET /api/v1/auth/verify-email
...
사용법:
    # 기본 (서버가 localhost:{{ cookiecutter.fastapi_port }}에서 실행 중이어야 함)
    python scripts/smoke_test.py

    # 사용자 지정 호스트/포트
    python scripts/smoke_test.py --host localhost --port {{ cookiecutter.fastapi_port }}
...
def step_health_check(cfg: SmokeTestConfig) -> None:
    ...
    status, data = http_get(url, timeout=cfg.timeout, allow_error=True)
    if status != 200:
        _die(
...
def step_register(cfg: SmokeTestConfig, state: TestState) -> None:
    ...
    url = f"{cfg.base_url}/api/v1/auth/register"
    _info(f"POST {url}")
...
def step_verify_email(cfg: SmokeTestConfig, state: TestState) -> None:
    """Mailpit API → 토큰 추출 → GET /api/v1/auth/verify-email?token=..."""
    ...
    # 인증 엔드포인트 호출
    # 쿼리 파라미터 방식
    verify_url = f"{cfg.base_url}/api/v1/auth/verify-email"
    params = urllib.parse.urlencode({"token": token})
    full_url = f"{verify_url}?{params}"
    _info(f"GET {verify_url}?token=<TOKEN>")

    status, data = http_get(full_url, timeout=cfg.timeout, allow_error=True)

    # 경로 파라미터 방식도 시도 (쿼리 방식 실패 시)
    if status not in (200, 201, 204):
        path_url = f"{cfg.base_url}/api/v1/auth/verify-email/{token}"
        _info(f"쿼리 방식 실패 ({status}), 경로 방식 시도: {path_url}")
        status, data = http_get(path_url, timeout=cfg.timeout, allow_error=True)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="{{ cookiecutter.project_name }} — 핵심 API 스모크 테스트",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument(
        "--host",
        default="{{ cookiecutter.fastapi_host if cookiecutter.fastapi_host != '0.0.0.0' else 'localhost' }}",
        help="FastAPI 서버 호스트 (기본값: localhost)",
    )
    parser.add_argument(
        "--port",
        type=int,
        default={{ cookiecutter.fastapi_port }},
        help="FastAPI 서버 포트 (기본값: {{ cookiecutter.fastapi_port }})",
    )
    parser.add_argument(
        "--mailpit-host",
        default="localhost",
    ...
    parser.add_argument(
        "--mailpit-port",
        type=int,
        default={{ cookiecutter.mailpit_ui_port }},
        help="Mailpit UI 포트 (기본값: {{ cookiecutter.mailpit_ui_port }})",
    )
    parser.add_argument(
        "--timeout",
        type=int,
...
def main() -> None:
    ...
    sep = "═" * W
    print(f"\n{_c(sep, '1;36')}")
    print(_c(f"  {{ cookiecutter.project_name }} — API 스모크 테스트", "1;32"))
    print(_c(sep, "1;36"))
...
{% else %}
    _section("단계 9-10 — 채팅 도메인 (비활성화)")
    _info("include_chat_domain=no 로 생성된 프로젝트입니다. 채팅 테스트를 건너뜁니다.")
{% endif %}
