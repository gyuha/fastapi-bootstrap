...
get_container_health() {
  ...
  # 컨테이너 상태 확인
  local status
  status=$(docker inspect --format='{% raw %}{{.State.Status}}{% endraw %}' "$container_id" 2>/dev/null || echo "unknown")
  ...
  # 헬스체크 상태 확인
  local health
  health=$(docker inspect --format='{% raw %}{{if .State.Health}}{{.State.Health.Status}}{{else}}none{{end}}{% endraw %}' \
  ...
  while true; do
    ...
    case "$health" in
      ...
      starting|none|unknown)
        ...
        spin_idx=$(( (spin_idx + 1) % {% raw %}${#spinner_chars[@]}{% endraw %} ))
        sleep "$POLL_INTERVAL"
        ;;
    esac
  done
...
# ── 메인 ────────────────────────────────────────────────────────────────────
main() {
  log_section "{{ cookiecutter.project_name }} — 서비스 기동 및 헬스체크"
  ...
  log_section "컨테이너 기동"
  log_info "${COMPOSE_CMD} -f ${COMPOSE_FILE} up -d"
  ...
  log_info "  헬스 엔드포인트      : curl http://localhost:{{ cookiecutter.fastapi_port }}/health"
}
