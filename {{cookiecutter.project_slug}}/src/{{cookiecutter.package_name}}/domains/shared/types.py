...
Usage::
    ...
    from {{ cookiecutter.package_name }}.domains.shared.types import UserId, ConversationId
...
#: Identity of a :class:`~{{ cookiecutter.package_name }}.domains.auth.models.User` aggregate.
UserId = NewType("UserId", uuid.UUID)

#: Identity of a :class:`~{{ cookiecutter.package_name }}.domains.auth.models.Role`.
RoleId = NewType("RoleId", uuid.UUID)

#: Identity of a :class:`~{{ cookiecutter.package_name }}.domains.auth.models.Permission`.
PermissionId = NewType("PermissionId", uuid.UUID)

{% if cookiecutter.include_chat_domain == "yes" %}
#: Identity of a :class:`~{{ cookiecutter.package_name }}.domains.chat.models.Conversation` aggregate.
ConversationId = NewType("ConversationId", uuid.UUID)

#: Identity of a :class:`~{{ cookiecutter.package_name }}.domains.chat.models.Message`.
MessageId = NewType("MessageId", uuid.UUID)
{% else %}
#: Placeholder — chat domain not included; defined for import compatibility only.
ConversationId = NewType("ConversationId", uuid.UUID)
MessageId = NewType("MessageId", uuid.UUID)
{% endif %}
