...
from {{ cookiecutter.package_name }}.core.exceptions import UnauthorizedError
...
async def blacklist_jti(redis: Redis, jti: str, ttl_seconds: int | None = None) -> None:  # type: ignore[type-arg]
    """Add *jti* to the Redis JWT blacklist."""
    key = f"{_BLACKLIST_PREFIX}{jti}"
    _ttl = ttl_seconds or (ACCESS_TOKEN_EXPIRE_MINUTES * 60 + 60)  # +1min buffer
    await redis.set(key, "1", ex=_ttl)
    logger.debug("jwt_blacklisted", jti=jti)
...
async def get_current_user(
    ...
    from {{ cookiecutter.package_name }}.domains.auth.models import Role, User  # noqa: PLC0415
