...
``LLM_PROVIDER`` from :class:`~{{ cookiecutter.package_name }}.core.config.LLMSettings` and
...
Architecture
------------
This module lives in the *infra* layer (``{{ cookiecutter.package_name }}.infra.llm``) rather
than inside a bounded context (``domains/<bc>/``) so that:

* The chat domain depends only on its ports/interfaces
  (:class:`~{{ cookiecutter.package_name }}.domains.chat.ports.LLMClientProtocol`) — never on
...
:class:`~{{ cookiecutter.package_name }}.core.config.LLMSettings` (``as_litellm_kwargs``).
...
def make_chat_litellm(
    ...
    **override_kwargs: Any,
    ...
    :class:`~{{ cookiecutter.package_name }}.core.config.LLMSettings` and constructs the
    ...
    ----------
    settings:
        Pre-populated :class:`~{{ cookiecutter.package_name }}.core.config.LLMSettings`
        instance.  When *None*, the global settings singleton is used (reads
        environment variables / ``.env`` file via
        :func:`~{{ cookiecutter.package_name }}.core.config.get_settings`).
    **override_kwargs:
